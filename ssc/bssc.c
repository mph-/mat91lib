...
static void bssc_advance (bssc_t *dev)
{
   ...
   /** advance the pointers for the DMA to use (ring isn't written via ring_write)
     ignore overrun for now */
   ring_write_advance (dev->ring, dev->dma_size);

   /** Update DMA pointer n stuff */
...
/** bssc_init
   ...
   @param ssc_cfg_t*   ssc configuration
   @param char* buffer to save to
   @param size, the size of the buffer in bytes
   @param dma_size, the size of each DMA transfer in bytes
   @return bssc_t*, the pointer to the buffered ssc structure
   */
void
bssc_init ( bssc_t* dev, const ssc_cfg_t *cfg, char *buffer, uint16_t size, uint16_t dma_size){
